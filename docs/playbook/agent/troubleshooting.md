...
| # | Issue | Symptom | Fix |
|---|-------|---------|-----|
| U1 | Missing `traefik.docker.network=coolify` | 504 Gateway Timeout | Add label; restart service — see [`../infrastructure/coolify-patterns.md`](../infrastructure/coolify-patterns.md) |
| U2 | Postgres hostname `postgres` on `coolify` network | App connects to Coolify's DB | Use unique alias (e.g. `litellm-postgres`) |
| U3 | DNS not ready before deploy | TRAEFIK DEFAULT CERT, ACME backoff | Add A record first; see `/opt/docs/troubleshooting.md` on Uranus |
| U4 | Coolify env vars empty after create | Containers start without secrets | `POST /services/{uuid}/envs` for each key — see [`../infrastructure/coolify-api.md`](../infrastructure/coolify-api.md) |
| U5 | Wrong Coolify API path | 404 on deploy | Compose stacks use `/services`, git apps use `/applications` |
| U6 | Zombie processes accumulating | Uptime Kuma / monitoring reports N zombies; `ps` shows `<defunct>` children | Diagnose parent PPID → container; use `init: true` or `tini` as PID 1 — see **Uranus: Docker Zombie Processes** below |
| U7 | `backup-runner` git/ssh zombies | Pairs of `[git]` + `[ssh]` under `tail -f /dev/null` after nightly Cronicle job | Rebuild with `tini` entrypoint in `/opt/infra/backup-runner/Dockerfile`; see U6 |
| U8 | `op run` leaves `[op]` zombie | Personal Dashboard (or any `op run -- …` wrapper) shows one `[op] <defunct>` while app is healthy | Resolve secrets with one-shot `op run … printenv` per key, then `exec` the app — not a long-running `op run` parent |

### C. Playwright / Testing
...
| 31 | CI e2e 403 / ECONNREFUSED (test origin ≠ server `PUBLIC_URL`) | Mutations silently fail **only in CI**; or `connect ECONNREFUSED` + 0 ms `beforeAll` | Make `BASE_URL` == `PUBLIC_URL` (host **and** port; use `127.0.0.1`); derive spec URLs from env |
...
### Solution
1. Navigate directly to the original source repository (e.g., `~/Projects/playbook`).
...
## 31. CI e2e Mutations 403 / ECONNREFUSED — Test Origin ≠ Server `PUBLIC_URL`

### Symptom
- e2e passes locally but fails **only in CI**. Browser-driven mutations (form submits — Confirm / Save / Approve) appear to do nothing: the POST fires but the page never reflects the change (an expected badge/row never appears), so the assertion times out with `element(s) not found`. Looks like flakiness or a render race.
- Or: `apiRequestContext.post: connect ECONNREFUSED 127.0.0.1:<port>`, and a spec's `beforeAll`/setup fails **every test at 0 ms**.

### Root Cause
A host/port mismatch between where the tests *originate* and where the server *expects* requests from — two distinct flavors, often together:

1. **CSRF origin 403.** The app's origin-check preHandler rejects any mutating request whose `Origin` header ≠ the server's expected origin (`PUBLIC_URL`). If CI points Playwright's `BASE_URL` at a different host than `PUBLIC_URL` — classically `localhost` vs `127.0.0.1` (same machine, **different string**) — every browser form-submit is silently `403`'d. `page.request` API calls frequently omit `Origin` and slip through, so **only the click-driven paths fail**, which is exactly why it masquerades as flakiness.
2. **Hardcoded port.** A spec hardcoding e.g. `http://127.0.0.1:3100` while CI runs the server on a different port (e.g. `3000`, derived from `BASE_URL`) → `ECONNREFUSED`.

`localhost` is also an IPv4/IPv6 trap: CI runners may resolve it to `::1`, but a server bound to `0.0.0.0` answers only IPv4 → `ECONNREFUSED ::1`.

### Detection
```bash
# Pull the failing CI job log and look for the tells
gh run view <run-id> --log-failed | grep -iE "ECONNREFUSED|Cross-origin|403|element\(s\) not found"

# Compare the THREE hosts/ports — they must ALL be byte-identical:
grep -n  "BASE_URL"            .github/workflows/*.yml   # what the tests target
grep -oE "PUBLIC_URL=[^ ]*"    package.json              # the server's expected Origin
grep -rnE "127.0.0.1:|localhost:" e2e/*.spec.ts          # any host/port hardcoded in a spec
```
You will **not** reproduce this locally if your local `BASE_URL`/default already matches `PUBLIC_URL` — it is CI-config-only. A local run is not a valid check for this class of bug; the PR's own CI run is.

### Solution
Make **one** host+port the single source of truth across all three:
- The CI workflow `BASE_URL`, the server's `PUBLIC_URL`, and any in-spec base URL must be **byte-identical** (`http://127.0.0.1:<port>`).
- Prefer `127.0.0.1` over `localhost` everywhere — IPv4-forced, and an exact-string Origin match.
- Specs must **derive** their base URL from `process.env.BASE_URL` (with a local default), never hardcode host/port: `const BASE_URL = process.env.BASE_URL ?? 'http://127.0.0.1:3100'`.

### Prevention
- Treat `BASE_URL` == `PUBLIC_URL` (host **and** port, exact string) as an invariant. The origin-check compares strings: `localhost` ≠ `127.0.0.1` even though they resolve to the same machine.
- Never hardcode a host/port in a spec — derive it from the same env the Playwright `webServer` / `baseURL` use.
- When you change a host string in one place (e.g. flipping `PUBLIC_URL` `localhost`→`127.0.0.1`), **grep for the other two and change them in the same commit.** A one-sided change is what creates this bug.

---

## Uranus: Docker Zombie Processes

*Fixed on Uranus 2026-06-18. See also [`../infrastructure/coolify-patterns.md`](../infrastructure/coolify-patterns.md) §5.*

### Symptom

Monitoring reports "N zombie processes" (e.g. Uptime Kuma). `top` shows zombies; they use no CPU or memory but the count grows over days.

```bash
ps -eo pid,ppid,user,stat,cmd | awk '$4 ~ /Z/'
ps -eo stat | grep -c Z
```

### Root cause

A **zombie** is an exited child whose parent never called `wait()`. In Docker, the problem is almost always **PID 1 not reaping orphans**:

| Bad PID 1 | Why it zombies |
|-----------|----------------|
| `tail -f /dev/null` | Keeps container alive but never reaps; `docker exec` children get reparented here |
| App process that spawns subprocesses without waiting | Short-lived children (git, ssh, node workers) stay in `Z` state |
| `op run --env-file=… -- node …` | `op` spawns a helper `[op]` child; long-running `op run` never reaps it |

`init: true` (Docker's built-in `docker-init`) or **`tini`** as entrypoint fixes cases where zombies are reparented to PID 1. It does **not** fix zombies whose parent is still alive (e.g. a running `op run`).

### Diagnose — map zombie → container

```bash
# List zombies with parent PID
ps -eo pid,ppid,user,stat,lstart,cmd | awk '$4 ~ /Z/'

# Identify parent command
ps -p <PPID> -o pid,ppid,stat,cmd

# Map host PID to container (parent of container PID 1 is containerd-shim)
sudo docker ps -q | while read cid; do
  hp=$(sudo docker inspect "$cid" --format '{{.State.Pid}}')
  echo "$cid $(sudo docker ps --filter id=$cid --format '{{.Names}}') host-pid=$hp"
done
```

**Uranus offenders (2026-06-18):**

| Zombies | Container | Parent | Trigger |
|---------|-----------|--------|---------|
| `[git]` + `[ssh]` pairs | `backup-runner` | `tail -f /dev/null` | Cronicle: `docker exec backup-runner python3 /scripts/sync_coolify.py` (git push) |
| `[node-MainThread]` | `litellm-cpjedj4g7asqcysohpcznzng` | litellm Python | Container restart / internal subprocesses |
| `[op]` | personal-dashboard | `op run … node` | Container start with 1Password injection |

### Fixes (by service type)

**Compose service (Coolify stack)** — add to service in `/opt/infra/coolify-services/<name>/docker-compose.yml` and live compose under `/data/coolify/services/<uuid>/`:

```yaml
init: true
```

Then recreate: `cd /data/coolify/services/<uuid> && sudo docker compose up -d --force-recreate <service>`

**Long-lived utility container (`backup-runner`)** — Dockerfile at `/opt/infra/backup-runner/Dockerfile`:
...
Rebuild: `cd /opt/infra/backup-runner && sudo docker compose build && sudo docker compose up -d --force-recreate`

**Git-built app with `op run` (personal-dashboard)** — do **not** wrap the app in a long-running `op run`. Resolve each key once, then `exec node`:

```sh
# docker-entrypoint.sh pattern (secrets may contain shell metacharacters — do not source injected files blindly)
grep -v '^\s*#' /app/op.env | grep -v '^\s*$' > "$op_env"
while IFS= read -r line; do
  key=${line%%=*}
  value=$(op run --env-file="$op_env" -- printenv "$key")
  export "${key}=${value}"
done < "$op_env"
exec node dist/index.js
```

Merged in [personal-dashboard PR #88](https://github.com/Performant-Labs/personal-dashboard/pull/88). Coolify redeploy: `GET /applications/hwe5u6kx1tyswj2mjt2bmm3n/restart` (queues build).

### Immediate cleanup

Restarting the affected container clears existing zombies (does not prevent recurrence):

```bash
sudo docker restart backup-runner
sudo docker restart litellm-cpjedj4g7asqcysohpcznzng
# personal-dashboard — prefer Coolify API restart so image rebuilds from git
```

### What not to do

- Do not `kill -9` zombies — they are already dead; only the parent or init can reap them.
- Do not patch only `/data/coolify/applications/.../docker-compose.yaml` without a Dockerfile fix for git apps — Coolify regenerates compose on redeploy.
- `op inject` + `source` the output fails when secrets contain characters that break shell parsing; use per-key `printenv` instead.
...
9a. **CI-only mutation 403 / `ECONNREFUSED`?** → Confirm `BASE_URL` (CI) == `PUBLIC_URL` (server) byte-for-byte, host **and** port; use `127.0.0.1`, derive spec URLs from env. See Issue #31
...
### Uranus / Docker
25. **Zombie process alert on Uranus?** → `ps … awk '$4 ~ /Z/'`, map PPID to container, apply `init: true` or `tini`; see **Uranus: Docker Zombie Processes** (U6–U8)

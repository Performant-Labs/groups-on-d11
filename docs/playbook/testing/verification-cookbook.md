...
#### 5. No Horizontal Overflow / Reflow (mobile-first)

**Rule:** any UI change is verified for horizontal overflow at the standard viewport set **before** any Tier 3 screenshot. This is a deterministic, structural (Tier 2) gate — not a visual judgement — and it's the cheapest way to catch the most common responsive defect (a fixed width, an unprefixed desktop grid, an unwrapped wide table).

```js
// Run via Playwright page.evaluate() at each width in {360, 768, 1280}.
for (const width of [360, 768, 1280]) {
  await page.setViewportSize({ width, height: 800 });
  const overflow = await page.evaluate(
    () => document.documentElement.scrollWidth - window.innerWidth,
  );
  // PASS when ≤ 0 — the page itself must not scroll sideways.
}
```

**Gates:**
- [ ] `scrollWidth − innerWidth ≤ 0` at 360, 768, and 1280.
- [ ] Wide content (tables, code, diagrams) is wrapped in `overflow-x-auto`, **never `overflow-hidden`** — confirm at Tier 1 (`grep`). A wide element inside `overflow-hidden` clips silently and does NOT register as document overflow, so the metric above is necessary but not sufficient.
- [ ] Exactly one `<meta name="viewport" content="width=device-width, initial-scale=1">` — no fixed `width=`, no `maximum-scale`/`user-scalable=no` (Tier 1 grep).
- [ ] Primary nav reachable below `md` (drawer/hamburger present and labelled).

This satisfies WCAG 2.2 SC 1.4.10 Reflow in practice. Full rule: [`../languages/css/responsive.md`](../languages/css/responsive.md); stack example: [`../frameworks/htmx/visual-regression-strategy.md`](../frameworks/htmx/visual-regression-strategy.md).

...
## Phase 5 (th scope a11y fix) — Feature Implementor (F)

**Decided:** Fixed the genuine AC-7/AC-15 accessibility defect T surfaced —
`ManageMembersForm::buildForm()`'s `$header` array entries changed from bare `TranslatableMarkup`
strings to the array cell form `['data' => $this->t(...), 'scope' => 'col']`, so Drupal's table
theme emits `<th scope="col">` for every column header. Minimal, header-only change — no
pagination/manager-service/config/`scope: outsider` logic touched.

**Assumed → verified, not added:** row-scope (`scope="row"`) was NOT added anywhere. Checked
`wireframe.md` (grep for `scope`) — only the column-header sketch is specified, no row-header
requirement. The failing test (`ManageMembersPageRenderTest::
testMemberListRendersAsRealTableWithScopedHeaders`) also asserts only `table th[scope="col"]`. Per
the "minimal, header-only" instruction, adding row-scope would be unrequested scope growth.

**Verified true (real execution, not static read):** stood up an isolated DDEV project
(`gm138-mc7`, distinct from the sibling `pl-groups-on-d11` project's own pre-existing exited
containers, which were never started/touched) and ran `ManageMembersPageRenderTest` for real — all
3 methods GREEN (19 assertions, 0 failures/errors). Tore the `gm138-mc7` project fully down
(containers/network/volumes/image) after; `docker ps -a` before/after confirms
`ddev-pl-groups-on-d11-{web,db}` were never touched.

**Tier 1:** phpcs (`Drupal,DrupalPractice`) 0 errors/warnings on the changed file; phpstan level 1
0 new findings (the sole reported item, "Unsafe usage of `new static()`" in `create()`, is
pre-existing — confirmed identical via `git stash`/re-run before and after this change, so not a
regression).

**Not touched:** the stale `docs/groups/modules/do_group_membership/src/Form/ManageMembersForm.php`
copy under `docs/groups/` (a separate, non-live planning-docs mirror last touched by commit
`5b8b08a`, not part of the PHPUnit-executed tree under `web/modules/custom/`) — confirmed via
`find . -iname "phpunit.xml*"` that the only PHPUnit config in the repo (`web/core/phpunit.xml.dist`)
resolves against `web/modules/custom`, so the `docs/groups/` copy is not exercised by any test and
editing it would be out of the minimal, header-only mandate.

**Evidence:** `handoff-F.md` "th scope a11y fix" section.

## Phase 8.6 — F edited the WRONG file (assembled artifact, not source) — caught before commit

**Decided:** F's th-scope fix landed in `web/modules/custom/do_group_membership/src/Form/
ManageMembersForm.php` (the assembled BUILD ARTIFACT, untracked, regenerated by assemble-config.sh)
— NOT in `docs/groups/modules/do_group_membership/src/Form/ManageMembersForm.php` (the SOURCE OF
TRUTH that gets committed and that CI assembles from). Verified on disk: source has 0
`'scope' => 'col'`, assembled has 5, and `web/modules/custom/...` is untracked (`git ls-files`
empty). So as-is the fix ships nowhere, the next assemble-config.sh OVERWRITES the assembled copy
from unfixed source, and CI's th-scope test FAILS. F's "3/3 GREEN" tested the edited assembled copy,
not the committed source — the #109 real-path trap, inverted. **Caught before committing.**

**Standing guardrail (added for all remaining F/T work on this story — patch-the-prompt):** edit ONLY
`docs/groups/` source; treat `web/modules/custom/` + `config/sync/` as regenerated build artifacts;
ALWAYS verify from the assembled layout (run assemble-config.sh first, then test) so you test what CI
tests, never the hand-edited artifact.

...
/**
 * Behavioral coverage for the daily digest drush command (#234, N-6).
 *
 * Part of the Notifications epic #237, N-6. Pins the
 * `Drupal\do_notifications\Commands\DigestCommands::digestDaily()` contract
 * (brief §"New objects" #4): claims every `frequency='daily'` queue row older
 * than a configurable window, groups by recipient, renders one aggregated
 * digest per user via N-8's `DigestRenderer` (reused verbatim), enqueues one
 * row per user into the NEW `do_notifications_digest_queue` table, deletes
 * the consumed source rows from `do_notifications_queue`, and skips users
 * with zero in-window items (no empty-digest email).
 *
 * Exercises AC1-AC9 from the brief:
 * - AC1: command is registered/callable (invoked directly here per the task
 *   prompt's kernel-test guidance — no drush shell-out from a kernel test).
 * - AC2/AC3/AC4: 10 daily items across 3 users -> 3 digest rows,
 *   window='daily', send_at=now, body content traceable to that user's
 *   messages, source rows deleted.
 * - AC5: an in-window daily item is not consumed.
 * - AC6: 'immediately'/'weekly' items older than the window are not consumed.
 * - AC7: a user with zero in-window items produces zero digest rows.
 * - AC8: command result summary shape.
 * - AC9: `state.set('do_notifications.digest_daily.window_seconds', N)`
 *   narrows the window.
 *
 * Also pins A's finding (handoff-A.md, "Omissions" #2, locked by the brief's
 * prompt): a queue row whose `mid` points to a deleted/non-existent Message
 * is dropped from the digest AND its queue row is deleted (garbage-collected)
 * rather than left to retry forever — and does not block digesting the
 * user's other valid messages.
 *
 * These are RED (Phase 4) tests: `DigestCommands`, `claimDaily()`,
 * `deleteByIds()`, `DigestQueueBackendInterface`, and the
 * `do_notifications_digest_queue` table do not exist yet. F implements
 * against this suite.
 *
...
class DailyDigestCommandTest extends GroupsKernelTestBase {

  /**
   * A fixed "now" used as the anchor for all created-time math in this suite.
   *
   * 2023-11-14 22:13:20 UTC. All "in window" / "out of window" timestamps
   * are computed relative to this value and `time.getRequestTime()` is not
   * mocked — see {@see self::assertRowsAssumeRealNow()} note below for why
   * that's safe for this suite's assertions.
   */
  private const FIXED_NOW_ANCHOR = 1700000000;

  /**
   * The default window (brief: 86400 = 24h), overridable via state.
   */
   ...
   *
   * Invoked directly (not shelled out via drush) per the task prompt's
   * kernel-test guidance: a kernel test has no drush binary/process
   * boundary, and calling the command class's method directly is both
   * faster and the idiomatic way to kernel-test a Drush 12+ attribute
   * command class.
   *
   ...
   * `created` timestamps deterministically relative to
   * {@see self::FIXED_NOW_ANCHOR} without a TestTime seam (none exists in
   * this codebase).
   *
   * @return int
  ...
  /**
   * The full scenario: 10 daily items (7/2/1 across 3 users) older than the
   ...
   * the task prompt (cheapest-sufficient-tier: one kernel test proving the
   ...
   * expensive setup).
   */
  public function testDigestDailyAggregatesPerUserAndConsumesSourceRows(): void {
    ...
    // 7 daily items, older than window, for user A.
    $user_a_message_ids = [];
    for ($i = 0; $i < 7; $i++) {
      $message = $this->createMessage($group, $author->id(), $old - $i);
      $user_a_message_ids[] = (int) $message->id();
      $this->insertQueueRow((int) $user_a->id(), (int) $message->id(), 'daily', $old - $i);
    }

    // 2 daily items, older than window, for user B.
    for ($i = 0; $i < 2; $i++) {
      $message = $this->createMessage($group, $author->id(), $old - 100 - $i);
      $this->insertQueueRow((int) $user_b->id(), (int) $message->id(), 'daily', $old - 100 - $i);
    }

    // 1 daily item, older than window, for user C.
    $message_c = $this->createMessage($group, $author->id(), $old - 200);
    $this->insertQueueRow((int) $user_c->id(), (int) $message_c->id(), 'daily', $old - 200);

    // 2 daily items INSIDE the window (must not be consumed).
    ...
    $this->insertQueueRow((int) $user_a->id(), (int) $in_window_message_1->id(), 'daily', $in_window);
    $this->insertQueueRow((int) $user_b->id(), (int) $in_window_message_2->id(), 'daily', $in_window - 1);
    ...
    // 2 'weekly' items, older than window (must not be consumed).
    $weekly_message_1 = $this->createMessage($group, $author->id(), $old - 7);
    $weekly_message_2 = $this->createMessage($group, $author->id(), $old - 8);
    $this->insertQueueRow((int) $user_b->id(), (int) $weekly_message_1->id(), 'weekly', $old - 7);
    $this->insertQueueRow((int) $user_c->id(), (int) $weekly_message_2->id(), 'weekly', $old - 8);

    // $user_zero has zero items at all.
    unset($user_zero);

    $this->assertSame(16, $this->sourceRowCount(), 'Sanity check: 16 rows total before running the command (7+2+1+2+2+2).');

    $result = $this->command()->digestDaily();

    // AC8: summary shape.
    $this->assertIsArray($result, 'digestDaily() returns an array summary.');
    $this->assertSame(3, $result['users_digested'], 'Exactly 3 distinct users had daily items in window.');
    $this->assertSame(10, $result['items_consumed'], 'Exactly 10 daily+in-window-threshold items were consumed (7+2+1).');
    ...
    foreach ($digest_rows as $row) {
      $this->assertSame('daily', $row['window'], 'Every digest row is tagged window=daily.');
    ...
    // AC4: user A's digest body traces to exactly 7 rendered fragments.
    $user_a_row = NULL;
    foreach ($digest_rows as $row) {
    ...
    $this->assertStringContainsString(
      '7 updates',
      $user_a_row['subject'],
      "User A's digest subject reports exactly 7 rendered updates (matches DigestRenderer's fragment count), proving the body was produced from user A's own 7 messages and not some other count.",
    );

    // AC3: the 10 consumed rows are gone; the 6 untouched rows remain.
    $this->assertSame(6, $this->sourceRowCount(), 'The 10 consumed daily/out-of-window rows are deleted; the 2 in-window + 4 other-frequency rows remain (6 total).');
    ...
    $expected_remaining = [
      ...
      (int) $weekly_message_1->id(),
      (int) $weekly_message_2->id(),
    ];
  ...
  /**
   * `state.set('do_notifications.digest_daily.window_seconds', N)` narrows
   * the window: a row that would be "in window" under the 24h default is
  ...
  public function testWindowSecondsStateOverrideNarrowsWindow(): void {
    ...
    $now = \Drupal::time()->getRequestTime();
    // 10 minutes old: within the DEFAULT 24h window (would not be consumed
    // by default), but OUTSIDE a compressed 5-minute window.
    $created = $now - 600;
    $message = $this->createMessage($group, $author->id(), $created);
    $this->insertQueueRow((int) $user->id(), (int) $message->id(), 'daily', $created);

    // Under the default window, this item is NOT consumed.
    $result_default = $this->command()->digestDaily();
    $this->assertSame(0, $result_default['items_consumed'], 'Under the default 24h window, a 10-minute-old item is still in-window and not consumed.');
    ...
    \Drupal::state()->set('do_notifications.digest_daily.window_seconds', 300);

    $result_narrowed = $this->command()->digestDaily();
   ...
   *
   * Pins A's finding (handoff-A.md "Omissions" #2, option (a), locked by the
   * task prompt): rather than leaving an orphaned row to retry forever, the
   * command garbage-collects it on first encounter.
   */
  public function testOrphanedMidIsDroppedAndDeletedWithoutBlockingOtherMessages(): void {
    ...
    $this->insertQueueRow((int) $user->id(), (int) $valid_message->id(), 'daily', $old);

    // A queue row pointing at a Message id that does not exist.
    $nonexistent_mid = 999999;
    $this->insertQueueRow((int) $user->id(), $nonexistent_mid, 'daily', $old - 1);
  ...
  public function testUserWithNoInWindowItemsProducesNoDigestRow(): void {
    ...
    $this->assertSame(0, $result['users_digested'], 'No users had any daily items at all.');
